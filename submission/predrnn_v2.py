...
class PredRNNModel(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.predrnn = RNN(config['numlayers'], config['hiddenlayers'], self.args)
        self.config = config
    ...
    def forward(self, x):
        ...
        x = reshape_patch(x, self.config['patchsize']).float()
        ...
        x = reshape_patch_back(x, self.config['patchsize']).float()
