...
class ConfigE3D(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        size = 128//(1 + int(bool(config['downsample'])))
        input_shape = (1, config['input_window'], size, size)
        output_shape = (1, config['output_window'], 64, 64)
        tau = 2
        hidden_size = config['hiddensize']
        kernel = (1, config['kernelsize'], config['kernelsize'])
        lstm_layers = config['lstmlayers']
        self.encoder = E3DLSTM(input_shape, hidden_size, lstm_layers, kernel, tau)
