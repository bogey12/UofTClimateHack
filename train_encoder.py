...
if __name__ == '__main__':
    ...
    ch_dataset = ClimateHackDataset(dataset, crops_per_slice=5, day_limit=7, outputs=N_IMS)
    datapoints = np.load('ds_total.npy', allow_pickle=True)
    datapoints = datapoints.reshape((len(datapoints), 3)).tolist()
    ch_dataset.cached_items = datapoints
    ch_dataloader = DataLoader(ch_dataset, batch_size=BATCH_SIZE, num_workers=1, persistent_workers=True)
    ch_dataloader.multiprocessing_context = 'spawn'
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # device = 'cpu'
    # print(device)
    model = Encoder(inputs=12, outputs=N_IMS, fac=1023, half_size=True).to(device)
    # model.load_state_dict(torch.load('submission/encoder8.pt'))
    optimiser = optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-8)#, momentum=0.9)
    criterion = MS_SSIMLoss(channels=N_IMS, data_range=1023)
    losses = []
    for epoch in range(EPOCHS):
        ...
        i = 0
        count = 0
        for batch_coordinates, batch_features, batch_targets in tqdm(ch_dataloader, total=len(datapoints)):
            # for j in range(12 - 3):
            optimiser.zero_grad()
    ...
    torch.save(model, f'submission/encoder2.pt')
