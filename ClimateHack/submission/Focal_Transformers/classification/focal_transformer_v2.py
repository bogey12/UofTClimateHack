...
class FocalTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            img_dim = int(x.shape[1] ** 0.5)
            b = rearrange(x, 'b (x y) c -> b c x y', x=img_dim, y=img_dim)
            skips.append(b)
            x = layer(x)
        x = self.norm(x)  # B L C
        img_dim = int(x.shape[1] ** 0.5)
        x = rearrange(x, 'b (x y) c -> b c x y', x=img_dim, y=img_dim)
...
class FocalTransformerSys(nn.Module):
    ...
    def __init__(self, 
        ...
        print("FocalTransformerSys expand initial----depths:{};depths_decoder:{};drop_path_rate:{};num_classes:{}".format(depths,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer ==0 :
                ...
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)),
                                ...
                                norm_layer=norm_layer,
                                pool_method=focal_pool if (self.num_layers-1-i_layer) in focal_stages else "none",
...
if __name__ == '__main__':
    img_size = 128
    x = torch.rand(16, 12, img_size, img_size).cuda()

    # focal tiny
    #model = FocalTransformer(img_size=img_size, patch_size=2, in_chans=12, embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
    ...
    model = FocalTransformerSys(img_size=img_size, patch_size=2, in_chans=12, embed_dim=48, depths=[2,2,6,2,1], drop_path_rate=0.2, 
